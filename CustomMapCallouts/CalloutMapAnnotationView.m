...
- (void)prepareFrameSize {
	...
	frame.size = CGSizeMake(270, height);
	self.frame = frame;
}
...
- (void) preventParentSelectionChange {
	EDBasicPinAnnotation *parentView = (EDBasicPinAnnotation *)self.parentAnnotationView;
...
- (void)adjustMapRegionIfNeeded {
	...
	if (xPixelShift || yPixelShift) {
        ...
        @catch (NSException *exception) {
            NSLog(@"Error calculating pixel shifted region: %@", exception);
        }
        @finally {
